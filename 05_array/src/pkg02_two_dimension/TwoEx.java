...
public class TwoEx {
  ...
  public static void main(String[] args) {
    ...
    // 2차원 배열의 초기화-1  (선언할때만 가능한 초기화 방식(중괄호를 쓴다))
    String[][] animals = {
        {"tiger", "lion",},
        {"dolphin", "shark"},
        {"falcon", "eagle"}
    ...
    for(int i = 0; i < animals.length; i++) {
      for(int j = 0; j < animals[i].length; j++) {
        System.out.print(String.format("%10s", animals[i][j])); // %10s -> 문자기준 10자를 준비하고 오른쪽 정렬
      }
      System.out.println();
    }
    System.out.println("-------------------");
    ...
    for(int i = 0; i < flowers.length; i++) {
      for(int j = 0; j < flowers[i].length; j++) {
        System.out.print(String.format("%-5s", flowers[i][j]));  //  %-5s -> 문자기준 5자를 준비하고 왼쪽 정렬
      }
      System.out.println();
    }
    System.out.println("-------------------");
